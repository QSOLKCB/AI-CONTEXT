...
def secret_scan(name: str, data: bytes) -> None:
    for pattern in SECRET_PATTERNS:
        for m in pattern.finditer(data):
            ...
            if line_end < 0: line_end = len(data)
...
def verify_zip(path: Path) -> dict:
    with zipfile.ZipFile(path, "r") as zf:
        ...
        if names != sorted(names):
            raise RuntimeError("source ZIP entries are not deterministically sorted")
        for info in zf.infolist():
            p = safe_member(info.filename)
            mode = (info.external_attr >> 16) & 0o170000
            if mode == 0o120000:
                raise RuntimeError(f"symlink in source ZIP: {info.filename}")
        ...
        expected = [n for n in names if n != "ARCHIVE-MANIFEST.json" and not n.endswith("/")]
...
def main() -> int:
    ...
    z = d / SOURCE_ZIP; p = d / OVERVIEW_PDF; n = d / RELEASE_NOTES
    verify_zip(z); compare_reference_against_tag(z); verify_pdf(p); verify_notes(n, sha256(z), sha256(p))
